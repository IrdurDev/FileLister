...
3. Click **"📂 + Subir archivos"** (Upload files) and select one or more files.
4. The files appear in a table with their names, sizes and dates.
5. Use the sorting buttons to reorder the list:
   - 🔤 Alfabético – alphabetical order
   - 🔢 Alfanumérico – natural alphanumeric order
   - 📅 Fecha – by modification date (newest first)
   - ⚖️ Tamaño – by file size (smallest first)
6. Click **"📄 Descargar .txt con la lista"** to export the current view as a text file.
7. Remove individual files with the ✖️ button, or use **"🗑️ Limpiar lista"** to remove everything.
